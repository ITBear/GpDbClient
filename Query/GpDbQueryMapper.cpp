                                       ...
                                       GpDbConnection&          aDbConn)
{
                                                                 ...
                                                                 [&](const GpTypeStructInfo&    aStructInfo)
    {
        ...
        builder
            ...
            .BRACE_BEGIN()
                .STRUCT_PARAM_NAMES(aStructInfo)
            .BRACE_END()
            .VALUES_BEGIN()
                .STRUCT_PARAM_BINDS(aStructInfo)
            .VALUES_END();
                                          ...
                                          const GpTypeStructBase&   aStruct)
{
    ...
    for (const GpTypePropInfo& propInfo: structInfo.Props())
    {
        THROW_GPE_COND_CHECK_M(propInfo.Container() == GpTypeContainer::NO,
                               "Container of property '"_sv + structInfo.Name() + "."_sv + propInfo.Name() + "' must be NO");
...
void    GpDbQueryMapper::SRowToStruct (const GpDbQueryRes&  aDbQueryRes,
                                       ...
                                       const count_t        aColOffset)
{
    ...
    for (const GpTypePropInfo& propInfo: structInfo.Props())
    {
        ...
        switch (propInfo.Type())
        {
            case GpType::U_INT_8:   propInfo.Value_UInt8(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<u_int_8>(); break;
            case GpType::S_INT_8:   propInfo.Value_SInt8(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<s_int_8>(); break;
            case GpType::U_INT_16:  propInfo.Value_UInt16(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<u_int_16>();   break;
            case GpType::S_INT_16:  propInfo.Value_SInt16(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<s_int_16>();   break;
            case GpType::U_INT_32:  propInfo.Value_UInt32(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<u_int_32>();   break;
            case GpType::S_INT_32:  propInfo.Value_SInt32(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<s_int_32>();   break;
            case GpType::U_INT_64:  propInfo.Value_UInt64(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<u_int_64>();   break;
            case GpType::S_INT_64:  propInfo.Value_SInt64(aStruct) = aDbQueryRes.GetInt64(0_cnt, colId++, 0_s_int_64).As<s_int_64>();   break;
            case GpType::DOUBLE:    THROW_GPE("Unsupported type DOUBLE"_sv); break;
            case GpType::FLOAT:     THROW_GPE("Unsupported type FLOAT"_sv); break;
            case GpType::BOOLEAN:   propInfo.Value_Bool(aStruct) = aDbQueryRes.GetBoolean(0_cnt, colId++, false);   break;
            case GpType::UUID:      propInfo.Value_UUID(aStruct) = aDbQueryRes.GetUUID(0_cnt, colId++, GpUUID());   break;
            case GpType::STRING:    propInfo.Value_String(aStruct) = aDbQueryRes.GetStr(0_cnt, colId++, ""_sv); break;
            case GpType::BLOB:      propInfo.Value_BLOB(aStruct) = GpBytesArrayUtils::SMake(aDbQueryRes.GetBLOB(0_cnt, colId++, {})); break;
            case GpType::STRUCT:    THROW_GPE("Unsupported type STRUCT"_sv); break;
            case GpType::STRUCT_SP: THROW_GPE("Unsupported type STRUCT_SP"_sv); break;
            case GpType::ENUM:      propInfo.Value_Enum(aStruct).FromString(aDbQueryRes.GetStr(0_cnt, colId++, std::nullopt));break;
            case GpType::NOT_SET:
            default:
            {
                THROW_GPE("Unsupported type NOT_SET"_sv); break;
...
void    GpDbQueryMapper::SJsonToStruct (GpDbQueryRes&       aDbQueryRes,
                                        ...
                                        const count_t       aColOffset)
{
    GpRawPtrCharRW jsonStrRW = aDbQueryRes.GetJsonStrRW(0_cnt, aColOffset, std::nullopt);
